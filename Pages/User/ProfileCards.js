...
import {
    getTimeDiff,
    normalize,
} from "../../nonVisualComponents/generalFunctions";
...
export default function ProfileCards({ navigation, route }) {
    ...
    const handleSwipe = ({ value, index }) => {
        ...
        console.log(
            "\nUSER SWIPED: " +
                shownList[index].UserId +
                " " +
                shownList[index].Name +
                "\n"
        );
        if (shownList[index].adCard != true)
            ...
            .catch((error) => {
                if (error.response) {
                    ...
                    if (error.response.status == 408) {
                        ...
                        updateProfile({
                            SwipeRefreshTime: error.response.data,
                        });
                        Session.LikeCount = 0;
        ...
        <View style={commonStyles.Container}>
            ...
            >
                <View style={styles.modal_container}>
                    ...
                    >
                        {Object.values(REPORT_REASONS).map(
                            ({ choice, key }) => {
                                return (
                                    <CustomButton
                                        key={key}
